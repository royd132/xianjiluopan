...
> **⚠️ 历史设计文档 — 仅供架构演进参考**
>
> 本文档（PRD v1.0，2026-08-13）记录的是项目最早期的产品设计与技术选型讨论。
> 其中提到的 **Streamlit 前端、LangGraph 编排、Gemini 模型、all-MiniLM Embedding** 等技术路线
> 已在后续开发中被完全替换。**复赛实现的真实技术栈以 [README](../README.md) 为准：**
>
> - 前端：**React 19 + Vite 8**（非 Streamlit）
> - 后端：**FastAPI + 自研事件驱动 Runtime**（非 LangGraph）
> - 模型：**阿里云百炼 Qwen 3.7**（非 Gemini）
> - 检索：**BM25 Skill Bank**（非 Embedding 向量）
>
> 本文档保留是为了展示从 MVP 构想到最终实现的演进过程。
> 如需了解当前系统状态，请阅读 [README](../README.md) 或 [复赛提交材料](../deliverables/)。

## —— 产品需求文档（PRD）v1.0（历史版本）
