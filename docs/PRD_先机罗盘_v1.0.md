...
| 术语 | 定义 |
|------|------|
| **决策卡 Decision Card** | 核心输出单元——带证据链、私域落点、反向条件、复核签字位的行动指令 |
...
| **私域落点 Private Domain Hook** | 决策卡强制字段，描述如何连接到具体私域人群和渠道 |
...
**先机罗盘不做又一个数据看板，而做一台发决策指令的机器——把全球多语言公开数据，编译成中小跨境卖家今天就能执行的选品方向、定价策略、竞争打法，并为每条结论附上可追溯的证据链与私域落点。**
...
- [ ] 卡片包含：行动指令 / 证据链（≥3条可下钻）/ 私域落点 / 反向条件 / 数据源披露 / 复核签字位
- [ ] 证据链中至少 1 条来自非英语数据源
- [ ] 缺少私域落点时卡片不生成，提示"无法确定私域落点"
- [ ] 反向条件 ≥ 1 条
...
```
输入(5s) → Agent并行分析(30-60s) → 四卡输出(即时) → 人工复核(可选) → 执行落地(私域渠道)
```
...
| 422 | CARD_REJECTED_MISSING_HOOK | 缺少私域落点 |
...
| 3:10 | 私域卡 | 人群卡 | "找养宠+夜班人群，WhatsApp养宠群，'让它半夜别吵醒你'。" |
...
评委第三眼（90-150s）: "跟私域有什么关系？"
  → ❌ NO! 每张卡**强制带私域落点**（缺字段=不生成）
  → **从洞察直连到"找谁、在哪、说什么"**
...
| W2 | 决策卡引擎 v1 | Schema 定义 + 编译Agent + 四卡渲染（至少选品卡+私域卡可用） | 研发 |
...
| 输出可执行的洞察 | 行动指令字段 + 私域落点 | 🟢P0 | ✅ 已设计 |
| 连接私域流量 | PrivateDomainHook 强制字段 | 🟢P0 | ✅ 已设计 |
| AI 内容合规 | EU AI Act §50 内嵌 | 🟢P0 | ✅ 已设计 |
| 创新性/差异化 | 5大亮点（决策卡范式/多语言痛点/供应链提前量/套利窗口/私域地图） | 🟢P0 | ✅ 已设计 |
...
| 私域落点 | Private Domain Hook | 决策卡强制字段：种子人群+承接渠道+话术钩子 |
